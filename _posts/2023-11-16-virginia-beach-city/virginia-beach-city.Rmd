...
![Figure 3: *Partisan Breakdown of Districts in Virginia Beach City*](citycouncilparty.svg){width="328"}
...
![Figure 4: *Map of Voting Precincts and City Council Boundaries in Virginia Beach City*](fig1ccandprecincts.png){width="328"}
:::

::: {style="flex-direction: column;"}
![Figure 5: *Map of Census Blocks and City Council Boundaries in Virginia Beach City*](fig2ccandblocks.png){width="328"}
...
In Virginia Beach City, minority populations tend to vote as a block for Democratic candidates. Figure 5 displays the vote percentage of Democrats and Republicans in each precinct; Democrats perform best in black/minority precincts. To quantify how cohesive a group of voters are, Figure 6 examines only precincts where the majority of residents in the precinct is a minority. The percentage of democratic votes in any given precinct trends similarly to the overall percentage of minority residents in the precinct, ranging from a .78 to 1.05 ratio.

::: {style="display: flex;"}
::: {style="flex-direction: column; margin-right: 20px;"}
![Figure 5: *Minority Population Percentage by Precinct*](fig5demvote.svg){width="328"}
:::

::: {style="flex-direction: column;"}
![Figure 6: *Percent of Democratic Votes Compared to Minority Population Percentages in Majority Minority Precincts*](fig6demperf.svg){width="328"}
...
Both were compiled to create overall vote splits by precinct.

### Runs and Simulations

We sample 20,000 districting plans for Virginia Beach City across fifteen independent runs of the [Sequential Monte Carlo algorithm](https://alarm-redist.org/redist/reference/redist_smc.html). No special techniques were needed to produce the sample.

### Constraints

We add the following constraints to our simulations:

``` r
cons <- redist_constr(map) 
cons <- add_constr_grp_hinge(cons, strength=20, vap_black, vap, tgts_group = c(0.55)) 
cons <- add_constr_grp_hinge(cons, strength=-20, vap_black, vap, tgts_group = 0.4)
cons <- add_constr_grp_hinge(cons, strength=-5, vap_black, vap, tgts_group = 0.2) 
```
...
However, we find that school zoning districts are an effective alternative to the missing historic neighborhoods. We utilize the [55 elementary school zoning districts](https://data.virginiabeach.gov/datasets/e185bc16ab9a4bbbaf2c2cfcf9156e78_25/about) to serve as a de-facto neighborhood map. School zoning districts usually correlate with historic areas of a city/county, and elementary school zoning districts are small enough to be beneficial for redistricting simulations while encompassing 2-3 neighborhoods.

![Figure 7: *Elementary School Zoning Boundaries Overlayed on City Council Districts. Elementary school boundaries are in blue, while City Council districs are in red.*](fig7eszoningr.svg)
