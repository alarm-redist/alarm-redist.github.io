...
We find that the NMF is too noisy to use alone, but the post-processing step of the TopDown algorithm reduces error substantially, making the post-processed data as accurate as swapping.
Although errors from privacy protection methods are generally smaller than other sources of census errors, they can be substantial for census geographies with small populations.
We also find that the average bias is fairly low across groups, but there is the more uncertainty and noise for Hispanic and multiracial people. Bias and RMSE estimates nationally for 5 geographic levels are displayed by race group below.
