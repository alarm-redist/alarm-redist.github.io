...
#' @examples
#' # TODO
plot_cds <- function(map, pl, county, abbr, qty = "plan", city = FALSE, coverage = FALSE) {
  if (n_distinct(pl) > 6) {
    plan <- redist:::color_graph(get_adj(map), as.integer(pl))
  } else {
    plan <- pl
  }
  places <- suppressMessages(tigris::places(abbr, cb = TRUE, progress_bar = FALSE))
  if (city) {
  ...
  map %>%
    ...
    summarize(
      .plan = .plan[1],
      dem = 1 / (1 + sum(rvote, na.rm = TRUE) / sum(dvote, na.rm = TRUE)),
...
download_dataverse <- function(slug) {
  ...
  load_file <- function(suffix = "stats.tab") {
    ...
    if (ext == "rds") {
      ...
      out <- read_csv(tf,
        col_types = cols(draw = "f", chain = "i", district = "i"),
