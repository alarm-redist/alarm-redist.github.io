...
We've generated `r comma(N)` sets of simulated districts according to [the relevant criteria](#redistricting-requirements).
Three of these districts are shown here, along with the actual enacted plan.
...
p1 = plot_cds(a$map, get_existing(a$map), county, abbr, "dem") + 
...
redist.plot.distr_qtys(a$plans, ndshare, color_thresh=0.5, size=0.05, alpha=0.5) +
    ...
    annotate("text", x=1.5, y=min(a$plans$ndshare[1:n_dists]), 
...
pbias_lim = c(min(-0.1, a$plans$pbias), max(0.1, a$plans$pbias))
egap_lim = c(min(-0.1, a$plans$egap), max(0.1, a$plans$egap))
...
In `r params$state`, the enacted plan is `r qile_english(d_comp$comp[1], comp_sims, "compact ")` simulated plans.
...
p_comp = d_comp %>%
    ...
    labs(x = "Average Polsby Popper (larger is more compact)") +
