...
<p style="text-align:right;">**日本語** / [**English**](en/index.html)</p>
...
```{r include=F}
prefs = Sys.glob(here("docs/japan-47-pref/analyses/*_*")) |>
    basename() |>
    substr(1, 2) |>
    as.integer()
```
...
d_appor = read_csv(here("docs/japan-47-pref/pref_code.csv"))

japan = readRDS(here("docs/japan-47-pref/carto_japan.rds")) %>%
    st_set_crs(NA) %>%
    ...
    mutate(available = pref_code %in% prefs,
           ...
           code = if_else(available, str_glue('window.open("{href}/index.html", "_self")'), ""),
...
filter(japan, available) %>%
    ...
    mutate(html = str_glue('<p><a href="{href}/index.html">{pref_japanese}</a></p>')) %>%
    pull() %>%
    cat(collapse="\n")
cat('</div><br />\n\n')
```
...
<pre class="citation-appendix short">Miyazaki, et al. (2022, April 6). 47-Prefecture Redistricting Simulations
